...
def predict_with_prophet(ticker):
    try:
        # 過去2年分のデータを取得して学習精度を高める
        stock = yf.Ticker(ticker)
        df = stock.history(period='2y')
        if df.empty: return None
        
        # Prophet用のデータ形式 (ds: 日付, y: 価格) に変換
        ...
        return forecast[['ds', 'yhat']].tail(31) # 今日＋未来30日分
    except:
        return None
...
def main_logic(user_input):
    ...
    if forecast is None: return "銘柄が見つからないか、AI解析に失敗しました。"
    ...
    res += "※AIは過去のパターンから複雑な曲線を計算しています。直線的な勢いだけではありません。"
